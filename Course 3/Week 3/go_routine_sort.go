...
func main() {
	...
	go BubbleSort2(arr, lo[0], hi[0], &wg, &mu)
	go BubbleSort2(arr, lo[1], hi[1], &wg, &mu)
	go BubbleSort2(arr, lo[2], hi[2], &wg, &mu)
	go BubbleSort2(arr, lo[3], hi[3], &wg, &mu)
...
func BubbleSort2(arr []int, low, high int, wg *sync.WaitGroup, mu *sync.Mutex) {
